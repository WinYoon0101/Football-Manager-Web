...
export const applicationApi = {
  ...
  // Lấy applications theo mùa giải (chờ duyệt)
  getBySeason: (seasonId: number) =>
    api.get<Application[]>(`/applications/season/${seasonId}`),

  // Lấy tất cả applications của một mùa giải (không phân biệt status)
  getAllBySeason: (seasonId: number) =>
    api.get<Application[]>(`/applications/season/${seasonId}/all`),
